...
def write_to_file(content: str, file_name: str) -> str:
    with open("configure/" + file_name, "a") as file:
        file.write(content)
...
def rewrite_rule_file(content: str, file_name: str) -> str:
    with open("configure/" + file_name, "w") as file:
        file.write(content)
    return f"Rewritten {file_name}"

@mcp.tool(name="read_rule_file", description="Read the current rules.txt. Returns empty string if file not found or file empty. Files' names are sqli.conf, xss.conf, path_traversal.conf, cmdi.conf,  Argument: file_name: str")
def read_rule_file(file_name: str) -> str:
    try:
        with open("configure/" + file_name, "r") as file:
            return file_name + "\n" + file.read()
